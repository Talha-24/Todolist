...
const CreateTask = () => {
  ...
  const   createTask=()=>{
    setnewTask({taskTitle,taskCategory,taskAssign,taskDate,taskDescription,newTask: true,failed: false,});
    console.log(newTask);
        ...
        <form onSubmit={(e)=>{e.preventDefault();}} className='flex  justify-between m-10 '>
            <div id="inputs" className='w-1/2   '>    
                ...
                <div>
                    ...
                    <input  onChange={(e)=>{settaskAssign(e.target.value)}} value={taskAssign} className='w-1/2 text-xl bg-[#1C1C1C] rounded-sm text-white border-[2px] border-purple-600 py-3 px-10' type="text" placeholder='e.g Talha,Uzair,Hammad' />
                </div>
                <div>
                    <h3  className='text-xl  font-bold mt-2'>Task Deadline</h3>
                    <input onChange={(e)=>{settaskDate(e.target.value)}} value={taskDate} className='w-1/2 text-xl bg-[#1C1C1C] rounded-sm text-white border-[2px] border-purple-600 py-3 px-10' type="datetime-local" />
                </div>
            ...
            <h3 className='text-xl  font-bold mt-2'>Task Description</h3>
                ...
                <textarea onChange={(e)=>{settaskDescription(e.target.value)}} value={taskDescription} className='h-full bg-[#1C1C1C] outline-none rounded border-[1px] border-red-600' name="" id="" cols="30" rows="10"></textarea>
                <button id="createBtn" onClick={createTask} className='  py-3 border-[2px] border-purple-600 text-purple-600 font-medium text-2xl mt-2 hover:bg-purple-600 hover: text-white rounded'>Create Task</button>
